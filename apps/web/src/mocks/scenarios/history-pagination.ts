...
const runs: ReviewRun[] = Array.from({ length: 21 }, (_, index) => ({
...
export function historyPagination(): RequestHandler[] {
  return [
    http.get(`${API}/workspaces/:workspaceId/review-runs`, ({ request }) => {
      const cursor = new URL(request.url).searchParams.get('cursor');
      return HttpResponse.json(cursor
        ? { items: runs.slice(20), next_cursor: null }
        : { items: runs.slice(0, 20), next_cursor: 'page-2' });
    }),
    ...happyPath(),
  ];
}
