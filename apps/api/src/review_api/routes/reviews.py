...
def list_runs(
    ...
    return request.state.platform.list_runs(workspace_id, cursor, limit)
...
async def create_run(
    ...
    if runtime is None:
        ...
        value = await anyio.to_thread.run_sync(
            request.state.platform.create_run, workspace_id, body, idempotency_key
        )
    else:
...
def get_run(request: Request, workspace_id: str, run_id: str):  # type: ignore[no-untyped-def]
    return request.state.platform.get_run(workspace_id, run_id).value
...
def cancel_run(request: Request, workspace_id: str, run_id: str):  # type: ignore[no-untyped-def]
    return request.state.platform.cancel_run(workspace_id, run_id)
...
def get_report(request: Request, workspace_id: str, run_id: str):  # type: ignore[no-untyped-def]
    body, etag = request.state.platform.report(workspace_id, run_id)
...
def list_finding_states(request: Request, workspace_id: str, run_id: str):  # type: ignore[no-untyped-def]
    return request.state.platform.states(workspace_id, run_id)
