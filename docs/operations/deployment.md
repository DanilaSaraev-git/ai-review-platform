...
Удалённый сервер в рамках этой интеграции не обновлён: вопрос о целевом стенде остаётся
без ответа. Последняя read-only SSH-проверка подтвердила current release
`3a0ab32c90f6e2aaf5b69e99b1875105761a2b3e`, четыре healthy сервиса и 16 GB свободного места.
При будущей выкладке потребуются миграции `0004` и `0005`, predeploy backup и отдельная
проверка совместимости отката: старый release ожидает схему `0003` и не подходит для
возврата после обновления БД. Серверный профиль модели сейчас не изменён.
