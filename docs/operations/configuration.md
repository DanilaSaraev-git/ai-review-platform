...
GPT-OSS 20B использует `response_format={"type":"json_schema"}` со строгой компактной
схемой отчёта. Версия профиля 1.0.2 заменила `json_object`, который гарантировал синтаксис
JSON, но пропускал ответы с неверным набором полей. Qwen3.6 35B пока использует
