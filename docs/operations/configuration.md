...
Review допускает до двух исправляющих генераций после невалидного JSON, ошибки схемы,
coverage, либо `finish_reason=length`. Каждая получает исходный документ и
...
диалог и решение человека остаются доступны. Summary, coverage и limitations модели
...
Ошибки JSON, схемы и coverage после исчерпания попыток завершают запуск ошибкой без отчёта.
