...
цитат или coverage, либо `finish_reason=length`. Каждая получает исходный документ и
безопасный код отказа с указаниями исправить результат. Сырой ответ модели не попадает
...
идентификатор фрагмента автоматически не исправляются. Неоднозначное совпадение отклоняется.
Невалидный ответ после исчерпания попыток по-прежнему завершает запуск ошибкой без отчёта.
