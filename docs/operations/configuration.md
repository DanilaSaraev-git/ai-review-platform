...
GPT-OSS 20B и Qwen3.6 35B используют нативный `response_format={"type":"json_object"}`: Yandex гарантирует
корректный JSON, а соответствие компактной схеме и каноническому контракту проверяет runtime
до публикации. Схема остаётся в доверенных инструкциях. DeepSeek пока использует `plain_json`.
