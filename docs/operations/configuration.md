...
Один файл может содержать несколько профилей одного провайдера с общей ссылкой на
credential. Существующий одиночный JSON остаётся совместимым. Для локального сравнения
DeepSeek с более дешёвыми базовыми моделями создайте набор так:

```sh
python3 tools/ops/render-yandex-profile.py \
  /absolute/private/path/yandex.folder-id \
  /absolute/private/path/yandex.multi-profile.json \
  --model gpt-oss20b --model qwen35b --model deepseek
```

На 6 сентября 2026 года профили используют доступные в базовом инстансе URI
`gpt-oss-20b`, `qwen3.6-35b-a3b` и `deepseek-v4-flash`. Тарифы синхронного режима
за 1000 входящих/исходящих токенов составляют соответственно 0,1/0,1 ₽,
0,2/0,3 ₽ и 0,3/0,5 ₽ с НДС. Точных Kimi K2, Qwen 2.5 32B и DeepSeek R1 14B
в базовом каталоге нет; ближайшие варианты для этого сравнения выбраны по размеру,
...
Шаблон GPT-OSS 20B версии 1.0.2 использует `response_format={"type":"json_schema"}`
со строгой компактной схемой отчёта. На локальном стенде после проверки механизма
восстановления и на проде активен профиль 1.0.1 с `json_object`: он гарантирует синтаксис
JSON, а обработка неверных полей выполняется runtime. Версия 1.0.2 включается отдельно;
обновление кода само по себе не заменяет приватный профиль. Qwen3.6 35B использует
`json_object`, DeepSeek — `plain_json`; соответствие компактной схеме и каноническому
контракту во всех режимах дополнительно проверяет runtime до публикации.
Параметр `max_completion_tokens=16384` ограничивает ответ вместе с reasoning;
...
Одиночный профиль активируется через `model-disable.sh`, `model-configure.sh`, `model-enable.sh`.
`model-configure.sh` пока не принимает набор: для него используется runtime-загрузчик
и процедура из [журнала подключения моделей](deployment.md#дополнение-каталога-моделей--2026-09-07).
...
Команда работает только с `REVIEW_COMPOSITION=ml`, проверяет каждую точную пару model profile
`id/version`, обращается только к объявленным `probe.url` методом GET и сохраняет отдельное
наблюдение для каждого профиля. Секрет читается из mounted file. Команда возвращает `0`
только когда все профили имеют свежее состояние `available`; отсутствующий probe, ошибка конфигурации, сети,
...
соединению клиента. Review имеет общий deadline 300 секунд, dialogue — 60 секунд. Один
сетевой вызов допускает не более одного автоматического повтора для 429/502/503/504 или
подтверждённой ошибки соединения до отправки, в пределах того же deadline.
Неопределённый timeout и content/context/auth failures автоматически не повторяются.

Review допускает до двух исправляющих генераций после невалидного JSON, ошибки содержательных полей
схемы либо `finish_reason=length`. Каждая получает исходный документ и
безопасную диагностику схемы с указаниями исправить результат. Предыдущий ответ передаётся
как недоверенные данные, если он помещается в прежний бюджет запроса; источники не обрезаются.
Сырой ответ модели не попадает
в доверенные инструкции или журналы. Используются прежние профиль модели, locale,
бюджет ответа, семафор и общий deadline; время не продлевается. Максимум — три генерации
и шесть сетевых обращений с учётом транспортных повторов. Все вызовы сохраняются в
`model_attempts`; usage в опубликованном отчёте относится к принятому ответу, а не к сумме
попыток. Отмена запрещает начало следующих попыток.

Различия только в пробелах, неразрывных пробелах и переносах строк цитаты восстанавливаются
детерминированно, если совпадение в указанном фрагменте единственное. В отчёт попадают
точная подстрока оригинала и её исходные offsets. Слова, числа, регистр, пунктуация и
идентификатор фрагмента автоматически не исправляются. По уточнённому поручению
пользователя неверная цитата или неизвестный фрагмент удаляют только привязку. Само
замечание, его тип, текст, вопрос и приоритет сохраняются. Если не осталось основной
привязки, `anchors` может быть пустым. Никакая искусственная цитата не подставляется.
В интерфейсе не добавляются метки или предупреждения о снятой привязке; карточка,
диалог и решение человека остаются доступны. Summary и limitations модели
не подменяются предупреждением об исключённых замечаниях. HTTP v1.2.0 явно допускает
пустые anchors у замечания о присутствующем тексте; опубликованные старые отчёты не меняются.
Перед проверкой содержательных полей runtime снимает Markdown-обёртку JSON, проецирует
известные поля, устраняет дубли и нормализует пустые или повреждённые списки ссылок.
У замечаний `missing` цитаты преобразуются в scope, затем ссылки проверяются сервером.
Пустые ссылки не запрещают публикацию; текст и число замечаний не меняются.
Coverage восстанавливается по серверному списку фрагментов: неизвестные IDs удаляются,
явно пропущенные фрагменты имеют приоритет над reviewed, все отсутствующие в ответе
фрагменты получают gaps. Полное покрытие не придумывается. Эти исправления не требуют
новой генерации. Ошибки JSON или содержательных полей после исчерпания попыток всё ещё
завершают запуск ошибкой без отчёта: отсутствующий текст замечания не сочиняется.
В журнале `review_output_rejected` сохраняются work item, номер попытки и безопасный код
отказа. Dialogue сохраняет прежнюю политику без исправляющих генераций.
...
а не гарантия завершения любого документа. Review использует ограниченное восстановление,
описанное в разделе «Исполнение и восстановление»; обрезанный JSON не публикуется.
