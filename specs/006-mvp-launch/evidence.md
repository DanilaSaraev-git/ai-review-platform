...
не является отдельным пользовательским экраном. Исторический compatibility gate — T044
(до объединения веток обозначался T040); после выбора Яндекса он отложен и не требует
...
T041 применима, только если отказ повторится с новой диагностикой.
...
T040 выполнена, агент не инициировал ни одной реальной генерации.
...
Отдельный серверный выпуск этой ветки не выполняется: задача гостевого доступа
уже объединяет те же принятые изменения и владеет общим deployment. Это исключает
конкурирующую публикацию ветки без новых миграций. Серверная визуальная проверка
будет выполнена после её общего выпуска; эта запись пока подтверждает локальный результат.
