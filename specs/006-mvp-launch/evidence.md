...
T044 остаётся открытой: для продолжения нужна отдельно разрешённая генерация,
поскольку существующий ответ нельзя повторно провалидировать offline.
Результат развёртывания диагностического изменения записывается после promotion.
