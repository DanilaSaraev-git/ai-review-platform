...
Дополнение OpenAI от 2026-09-06 подготовлено локально и описано в конце документа.
Оно не выкладывалось на сервер; работоспособность реальным запросом не проверялась.
Приведённые ниже результаты тестов и генерации относятся к прежним этапам и Kimi.
...
## OpenAI — подключение MVP 2026-09-06 без реальных запросов

Добавлены [профиль OpenAI](../../docs/operations/configuration.md#профиль-openai-для-mvp),
поддержка параметров и транспортной schema в адаптере; `./dev start` выбирает OpenAI,
`--model kimi` сохраняет Kimi. Полная валидация, engines и storage сохранены.
Ключ пока не настроен: проверено только наличие, значения не читались. Активация —
приватный `~/.config/ai-analytics-review/openai.token` (600, каталог 700) и `stop`/`start`.
Статически проверены синтаксис Python/JSON, `git diff --check` и `CLAUDE.md -> AGENTS.md`.
Тесты не добавлялись и не запускались; бенчмарки, probes, пробные API-запросы, платные
вызовы, запуск сервисов и deployment не выполнялись по поручению пользователя.
**Работоспособность OpenAI реальным запросом не проверялась.** Прежнее evidence Kimi
к нему не относится. Серверная активация потребует сначала версии с обновлённым адаптером.
