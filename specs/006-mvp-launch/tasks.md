...
Без нового design cycle, тестов, бенчмарков, probes, API-вызовов и deployment по поручению пользователя. T040 остаётся отдельным продолжением Kimi.

- [X] T041 Добавить профиль OpenAI `gpt-5.4-mini` с заданными параметрами и минимальную проекцию транспортной schema; сохранить полную валидацию, engines, anchors и storage.
- [X] T042 Выбрать OpenAI по умолчанию, сохранить `--model kimi`, подключить серверный файловый секрет; обновить настройки запуска и SpecKit с явным отсутствием реальной проверки OpenAI.
