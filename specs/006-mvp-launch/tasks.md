...
- [X] T039 Проверять finish reason до разбора ответа в review/dialogue/CLI: не публиковать даже валидный JSON с length, сохранить public error enum и запрет auto retry; пройти regressions и зафиксировать эксплуатационные границы. Код `1ec8acd` проверен локально, сохранён при объединении с Яндексом и выпущен на сервер в `9a4d767a8dcd30469a942d1f39b7f0751d5421a3`; инфраструктурная проверка прошла, новые генерации не выполнялись.
- [ ] T040 Отложено после выбора Яндекса: первоначально планировалось после восстановления API-кредитов HF продолжить диагностику validation_failed и пройти успешный исходный review. На момент инцидента HF возвращал HTTP 402; покупки не выполнялись. Новые генерации по этому историческому плану без отдельного поручения не выполняются. Completion guard выпущен по T039; повторный semantic отказ с Яндексом отслеживается отдельно в T044.
...
- [X] T042 Выпустить обновление на базе текущего production release, сохранить предыдущие model files, включить профиль Яндекса и проверить только GET каталога и инфраструктурную readiness.

## Phase 17: Диагностика отказа пользовательского запуска — 2026-09-06

- [X] T043 Сохранять фиксированную безопасную причину semantic validation, исправить terminal state и трактовку retryable в UI; проверить на synthetic fixtures и выпустить без генераций. Диагностический release `efe06f13c6f0c52d164b1c34df5e6fd81bc238f8` установлен; результаты записаны в evidence.
- [ ] T044 При повторении semantic отказа на новом выпуске разобрать сохранённую причину и проверить необходимое исправление. Исходный raw response не сохранён, offline replay этого запуска невозможен; следующий пользовательский запуск прошёл, дополнительная генерация для диагностики сейчас не требуется. Новая агентская генерация требует отдельного поручения.
...
| Этап | Номера во входящей ветке | Номера после объединения |
| --- | --- | --- |
| Подключение Яндекса | T038–T039 | T041–T042 |
| Диагностика semantic отказа | T040–T041 | T043–T044 |
| Исторический инцидент Kimi | T042–T044 | T038–T040 |
