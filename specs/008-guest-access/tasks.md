...
- [X] T014 Подготовить проверенную версию и резервную копию для выпуска, применить миграцию и обновить существующий сервис с сохранением runtime volumes. Release `3a0ab32c90f6e2aaf5b69e99b1875105761a2b3e`, schema `20260906_0003`; promotion с guest=false и переключение на true прошли. Backup `20260906T134547Z` восстановлен изолированно и скопирован вне VPS.
- [X] T015 Проверить реальный HTTPS-вход без кода, отдельные браузерные сессии, приватность `/demo` и persistence; записать release и результат, затем завершить срез. Два HTTPS cookie jars изолированы, identity/SHA файла сохранены после restart; UI проверен CUA через SSH-туннель, внешний HTTPS — curl через en0. Прямой browser HTTPS не заявляется пройденным.
...
Общий локальный gate пройден: backend 397 passed / 11 skipped (opt-in Docker сценарии отдельно пройдены), Ruff, Mypy 110, contract tooling и web проверки. T014–T015 завершены фактическим выпуском, HTTPS/persistence проверками и isolated restore. В evidence отдельно обозначено ограничение прямого browser HTTPS.
